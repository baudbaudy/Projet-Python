    ...
    def ajouter_un_livre(nouvelle_entree, nouveau_titre, nouvel_auteur, lettre_serie, numero_cote, maison_edition, commentaire, theme_ouvrage, type_ouvrage):
       #Fonction qui permet d'ajouter un livre au catalogue de la bibliothèque
        ...
        if not nouvelle_entree:
            erreurs.append("le champ 'ENTREE' doit être rempli")
        if not nouveau_titre:
            erreurs.append("le champ 'TITRE' doit être rempli")
        if not nouvel_auteur:
            erreurs.append("le champ 'AUTEUR' doit être rempli")
        if not maison_edition:
            erreurs.append("le champ 'EDITION' doit être rempli")
        if not(lettre_serie):
            erreurs.append("le champ 'SERIE' doit être rempli")
        if not(numero_cote):
            erreurs.append("le champ 'COTE' doit être rempli")
        if len(erreurs) > 0:
